...
template <int nBucketCount, class tKey = std::
uintptr_t>
class CUtlTSHashGenericHash {
public:
	static int Hash(const tKey& Key, int nBucketMask) {
		int nHash = HashIntConventional(std::
uintptr_t(Key));
...
template <class tElement, int nBucketCount, class tKey = std::
uintptr_t, class tHashFuncs = CUtlTSHashGenericHash<nBucketCount, tKey>, int nAlignment = 0>
class CUtlTSHash {
...
public:
	...
	UtlTSHashHandle_t Find(tKey uiKey) {
		// Hash() expects the mask - passing nBucketCount yields index ?
		// {0, nBucketCount} -> one-past-end bucket read -> garbage chain walk
		int iBucket = tHashFuncs::Hash(uiKey, nBucketMask);
...
private:
	...
	struct HashBucket_t {
	private:
		// Swap the MEM_PADs in
		// case of crash here, not sure exactly what this is all about - basically ignore
