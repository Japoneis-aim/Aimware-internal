#pragma once
#include <cstdint>
#include "../../../../Aimware/utils/memory/vfunc/vfunc.h"
...
#include "..\..\..\..\Aimware\utils\math\utlvector\utlvector.h"
...
struct SchemaClassFieldData_t {
	...
	std::
uint32_t nSingleInheritanceOffset; // 0x0010
	std::
int32_t nMetadataSize; // 0x0014
...
int16_t nFieldSize;        // 0x0024
	std::
int16_t pad_0x26;          // 0x0026
	std::
int16_t nStaticSize;       // 0x0028
	std::
int16_t nMetadataSize;     // 0x002A
	std::
uint8_t nAlignOf;          // 0x002C
	std::
uint8_t nBaseClassesCount; // 0x002D
	std::
int16_t pad_0x2E;          // 0x002E
...
public:
	...
	std::
int8_t nAlign_; // 0x0018
	MEM_PAD(0x3); // 0x0019
	std::
int16_t nSize_; // 0x001C
	std::
int16_t nFlags_; // 0x001E
	SchemaEnumeratorInfoData_t* pEnumInfo_;
	MEM_PAD(0x8); // 0x0028
	CSchemaSystemTypeScope* pTypeScope_; // 0x0030
	MEM_PAD(0x8); // 0x0038
	std::
int32_t unk1_; // 0x0040
};
...
public:
	...
	CSchemaType* FindSchemaTypeByName(const char* szName, std::
uintptr_t* pSchema);
