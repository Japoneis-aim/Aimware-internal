...
public:
	...
	schema(std::
uint32_t, m_nTickBase, "CBasePlayerController->m_nTickBase");
	...
	schema(std::
uint64_t, m_steamID, "CBasePlayerController->m_steamID");
	// Dump client_dll: money @ 0x810, rank @ 0x888
	schema(void*, m_pInGameMoneyServices, "CCSPlayerController->m_pInGameMoneyServices");
	schema(std::
int32_t, m_iCompetitiveRanking, "CCSPlayerController->m_iCompetitiveRanking");
	schema(std::
int32_t, m_iCompetitiveWins, "CCSPlayerController->m_iCompetitiveWins");
...
public:
	schema(std::
int32_t, m_iAccount, "CCSPlayerController_InGameMoneyServices->m_iAccount");
};
