...
namespace {
	...
	// SetParticleSettings - IDA 0x1809C5540
	...
	// world_effects: custom .vpcf_c -> csgo/bin/, spawn via CacheParticleEffect
	...
	// rain_edge = EDGE-only camera fill (rain only at screen borders)
	// rain_storm_screen= full-view screen rain (main visible layer)
	// rain_edge_sparse_longdrop = long visible streaks across the view
	// rain_volume = camera volume rain
	// rain_sheet = near-camera streak sheet
	...
	constexpr const char* kRainScreen = "particles/rain_fx/rain_storm_screen.vpcf";
	constexpr const char* kRainLong   = "particles/rain_fx/rain_edge_sparse_longdrop.vpcf";
	...
	// rain_fx storm/rain ignore CP0 after spawn (CreateWithinSphereTransform) -
	// the grid must be re-rooted around the player when they outrun the sphere.
	// Camera fills (MAIN_VIEW) keep the screen wet between re-roots.
	constexpr float kRainRecreateDistSq = 120.f * 120.f;

	void* GetMgr() {
		// GetGameParticleManager() first
		if (g_getParticleMgr) {
			__try {
	...
	// Prefer SetParticleSettings; fall back to SetEffect F3
	char SetCpF3Seh(void* mgr, int index, unsigned int cp, float a, float b, float c) {
		if (!mgr || index == kInvalidFx || index <= 0 || !g_setCpF3)
			return 0;
		float v[3] = { a, b, c };
		char ok = 0;
		// IDA 0x1809C58E0 = SetEffect float3. Do NOT call 0x1809C5940 here -
		// that is transform-set; passing a vec3 left CP0 unset (invisible weather).
		__try {
		...
		int attach, void* entity)
	{
		...
		// Custom particles: CP0 = world origin only (density set by CreateKind on CP2)
	...
	bool CreateKind(int kind, float intensity01) {
		...
		void* mgr = GetMgr();
		if (!mgr || !g_cacheParticle || !g_setCpF3) {
			++g_createFails;
			g_activeKind = 0;
			return false;
		}
		...
		if (kind == 1) {
			...
			// Camera volumes first - full-view rain (screen + long streaks +
			// sheet + volume). rain_edge is EDGE-only (borders) - skipping it
			// gives rain across the whole view like snow/stars.
			(void)spawn_rain(kRainScreen, kAttachMainView, 0.f, 0.f, 0.f);
			(void)spawn_rain(kRainLong,   kAttachMainView, 0.f, 0.f, 0.f);
		...
		// Re-root anchor for rain - world emitters read CP0 at spawn only
		g_spawnX = g_haveLastOrigin ? g_lastX : 0.f;
		g_spawnY = g_haveLastOrigin ? g_lastY : 0.f;
		g_spawnZ = g_haveLastOrigin ? g_lastZ : 0.f;
	...
	uintptr_t FindMapInfoEntity() {
		...
		// CMapInfo is always spawned in the first 512 world entities
		const int searchMax = (nMax < 512) ? nMax : 512;
		for (int i = 0; i <= searchMax; ++i) {
			...
			void* ident = nullptr;
			if (Mem::ReadField(ent, Offset::m_pEntity(), ident) && ident)
				(void)Mem::ReadField(ident, Offset::m_designerName(), designer);
			if (designer && designer[0]
				&& !std::strstr(designer, "map") && !std::strstr(designer, "Map"))
				continue;
			char clsName[128]{};
			if (!Mem::SchemaClassName(ent, clsName, sizeof(clsName)))
				continue;
			if (hash_32_fnv1a_const(clsName) == want)
...
void World::Weather::Install()
{
	...
	// SetEffect float3 - IDA 0x1809C58E0 (particle_set_control_point).
	...
	if (!g_setCpF3)
		Con::OffsetMiss("Weather SetEffect float3");
	else
		Con::Ok("Weather setCP f3=%p", (void*)g_setCpF3);
	...
	// Gate on resolved patterns - never spawn with half-resolved
	// particle fns (stale patterns on game update -> engine particle crash).
	g_ready = (g_cacheParticle != nullptr) && g_setCpF3 && g_destroyParticle;
...
void World::Weather::Update()
{
	...
	// Never touch the particle manager with half-resolved patterns 
	if (!g_ready)
		return;

	{
		std::lock_guard<std::mutex> lock(g_warmMutex);
		if (!g_warmPending && g_warmed.size() < 4)
			g_warmPending = true;
	}
	...
	if (reset) {
		...
		g_activeKind = 0;
		g_cachedMapInfo = 0;
		g_mapInfoFailStreak = 0; // force mapinfo rescan for wetness
	}
	...
	// never create/set CP until scene origin exists. Round recycle
	// leaves pawn alive with a null/stale node - CreateKind then AVs.
	...
	const float dx = o.x - g_lastX;
	const float dy = o.y - g_lastY;
	const float dz = o.z - g_lastZ;
	const bool moved = !g_haveLastOrigin || (dx * dx + dy * dy + dz * dz > 0.04f);
	...
	// Rain only: storm/rain/outer ignore live CP0 (spawn-time sphere) - tear
	// down + re-spawn the grid around the player once they outrun the old
	// sphere. Snow/stars/ash follow via live CP0 and never hit this path.
	if (kind == 4 && g_fxCount > 0 && g_spawnX != 0.f) {
		const float ddx = o.x - g_spawnX;
		const float ddy = o.y - g_spawnY;
		if (ddx * ddx + ddy * ddy > kRainRecreateDistSq) {
			if (g_recreateCooldown > 0) {
				--g_recreateCooldown;
			} else {
				g_recreateCooldown = 20;
				DestroyAll();
				(void)CreateKind(kind, intensity);
			}
		}
	...
	if (!g_cachedMapInfo) {
		...
		if (g_mapInfoFailStreak < 8 && --s_scanCooldown <= 0) {
			s_scanCooldown = 64;
	...
	} else if (!Mem::ValidEntity(reinterpret_cast<void*>(g_cachedMapInfo))) {
		// Entity freed/recycled between rounds - never write into stale memory
		g_cachedMapInfo = 0;
		g_mapInfoFailStreak = 0;
	}
	// Wetness for rain only
...
{
	// Leave / map unload - drop indices only. DestroyAll talks to a dying
	// particle manager; EnsureParticleAssets hits ResourceSystem.
	for (int i = 0; i < g_fxCount; ++i)
		g_fx[i] = FxSlot{};
	g_fxCount = 0;
	g_activeKind = 0;
...
{
	// Idle-inject: never touch the particle manager when weather feature is off.
	// (was: spawned+destroyed assets every FSN until warm-done - join-time FPS stutter)
	if (!Config::weather || Config::weather_mode < 1 || Config::weather_mode > kWeatherModeMax)
		return;
	...
	constexpr const char* kAll[] = {
		...
		kRainStorm, kRainMain, kRainOuter, kRainVol, kRainScreen, kRainLong, kRainSheet
	};
