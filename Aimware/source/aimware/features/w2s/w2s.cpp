...
bool g_inited = false;
std::uint64_t g_nextInitAttemptMs = 0;
...
bool SehCopyMatrix(const viewmatrix_t* src, viewmatrix_t* dst) {
	if (!src || !dst || !Mem::
IsReadable(src, sizeof(viewmatrix_t)))
		return false;
	__try {
		std::
memcpy(dst, src, sizeof(viewmatrix_t));
		return true;
...
bool SehScreenTransform(const Vector_t& world, Vector_t& ndcOut, bool& behind) {
	...
	__try {
		using Fn = std::
int64_t(__fastcall*)(const Vector_t*, Vector_t*);
		Vector_t in = world;
		Vector_t out{};
		const std::
int64_t ret = reinterpret_cast<Fn>(g_fnScreenTransform)(&in, &out);
		behind = (ret != 0);
		ndcOut = out;
		return std::
isfinite(out.x) && std::
...
sqrt(dx * dx + dy * dy);
	...
	const float maxHX = (std::
max)(cx - margin, 1.f);
	const float maxHY = (std::
max)(cy - margin, 1.f);
	float t = 1e9f;
	if (std::
fabs(dx) > 1e-6f)
	...
	float margin, float& outX, float& outY)
{
	...
	const float horiz = std::
sqrt(dxw * dxw + dyw * dyw);

	float screenDx = 0.f;
	float screenDy = -1.f;
	if (horiz > 1e-3f) {
		const float targetYaw = std::
atan2(dyw, dxw) * kRad2Deg;
...
cos(dyaw);
	...
	if (horiz > 1e-3f || std::
fabs(dzw) > 1e-3f) {
		const float targetPitch = -std::
atan2(dzw, (std::
max)(horiz, 1e-3f)) * kRad2Deg;
...
bool Init() {
	if (g_fnScreenTransform && g_matrixBase)
		return true;
	const std::uint64_t nowMs = static_cast<std::uint64_t>(GetTickCount64());
	if (g_inited && nowMs < g_nextInitAttemptMs)
		return g_fnScreenTransform != 0 || g_matrixBase != nullptr || g_hasLive;
	g_inited = true;
	g_nextInitAttemptMs = nowMs + 1000;
	...
	if (!g_matrixBase) {
		if (uintptr_t site = ScanClient("pViewMatrix", "48 8D 0D ? ? ? ? 48 C1 E0 06")) {
			const uintptr_t abs = M::
getAbsoluteAddress(site, 3, 0);
			if (abs)
...
void OnMatrixCaptured(const viewmatrix_t* worldToProjection) {
	...
	if (!worldToProjection || !Mem::
IsReadable(worldToProjection, sizeof(viewmatrix_t)))
		return;
...
static bool MatrixLooksValid(const viewmatrix_t& m)
{
	const float w00 = m.matrix[0][0];
	const float w33 = m.matrix[3][3];
...
bool HasCamera() {
	viewmatrix_t tmp{};
	if (g_hasLive && SehCopyMatrix(&g_live, &tmp) && MatrixLooksValid(tmp))
		return true;
	if (g_matrixBase && SehCopyMatrix(g_matrixBase, &tmp) && MatrixLooksValid(tmp))
		return true;
	return false;
}
