...
GameEntity->Instance) {
		...
		__try {
			ctrl = I::
GameEntity->Instance->Get<CCSPlayerController>(hCtrl);
			if (ctrl && Mem::
ValidEntity(ctrl)) {
		...
		if (ctrl) {
			if (hObs.valid()) {
				C_CSPlayerPawn* op = nullptr;
				__try { op = I::
GameEntity->Instance->Get<C_CSPlayerPawn>(hObs); }
				__except (EXCEPTION_EXECUTE_HANDLER) { op = nullptr; }
				if (op && Mem::
ValidEntity(op) && ObsOf(op))
					return op;
			}
			if (hPawn.valid()) {
				C_CSPlayerPawn* pp = nullptr;
				__try { pp = I::
GameEntity->Instance->Get<C_CSPlayerPawn>(hPawn); }
				__except (EXCEPTION_EXECUTE_HANDLER) { pp = nullptr; }
				if (pp && Mem::
ValidEntity(pp) && ObsOf(pp))
					return pp;
			}
		}
	}
...
// All alive players except local. Prefer OnAdd controller list (O(players)).
// Fallback: low-slot seed if list empty (mid-inject) - never full 1..highest walk.
...
GameEntity->Instance, 0x2100))
	...
	int ctrlIdx[SdkPrioA::
kMaxTrackedControllers]{};
	int nCtrl = SdkPrioA::
CopyControllerIndices(ctrlIdx, SdkPrioA::
kMaxTrackedControllers);
	if (nCtrl <= 0) {
		// Cold inject: seed controllers from low slots once (same as aim_common)
		const int nMaxRaw = I::
GameEntity->Instance->GetHighestEntityIndex();
		const int seedMax = (nMaxRaw < 72) ? nMaxRaw : 72;
		for (int i = 1; i <= seedMax && nCtrl < SdkPrioA::
kMaxTrackedControllers; ++i) {
			void* entRaw = nullptr;
			__try { entRaw = I::
...
Valid(id, 0x28)) {
					...
					if (!Mem::
ReadField(id, Offset::m_designerName(), designer) || !designer)
						designer = id->m_designerName();
					if (designer && Mem::
IsReadable(designer, 2) && designer[0]
						&& (std::
strcmp(designer, "cs_player_controller") == 0
							|| std::
strstr(designer, "player_controller") != nullptr))
						isCtrl = true;
				}
			} __except (EXCEPTION_EXECUTE_HANDLER) {
				isCtrl = false;
	...
	int count = 0;
	for (int ci = 0; ci < nCtrl && count < maxOut; ++ci) {
...
ValidEntity(pawn))
		...
		CBaseHandle hSelf{};
		__try { hSelf = pawn->handle(); }
		__except (EXCEPTION_EXECUTE_HANDLER) { hSelf = {}; }
...
void ApplyToPawn(C_CSPlayerPawn* pawn, const CBaseHandle& h, std::
uint8_t mode)
{
...
bool Init()
{
	...
	g_edgePrev = false;
	Con::
Ok("EnemySpec ready (LMB/RMB cycle, all alive players)");
	return true;
}
...
SafeLocalPlayer();
	...
	const std::
uint8_t mode = Config::
enemy_spectate_thirdperson ? kObsChase : kObsInEye;
	...
	if (hCtrl.valid() && I::GameEntity && I::GameEntity->Instance) {
		...
		if (ctrl && Mem::ValidEntity(ctrl)) {
			CBaseHandle hObs{};
			__try { hObs = ctrl->m_hObserverPawn(); }
			__except (EXCEPTION_EXECUTE_HANDLER) { hObs = CBaseHandle{}; }
			if (hObs.valid()) {
				C_CSPlayerPawn* op = nullptr;
				__try { op = I::GameEntity->Instance->Get<C_CSPlayerPawn>(hObs); }
				__except (EXCEPTION_EXECUTE_HANDLER) { op = nullptr; }
				if (op && op != specPawn && op != live && Mem::ValidEntity(op))
