...
// Map leave - drop pawn*/boneArray cache. Recycled skeletons reuse addresses.
void Invalidate();
...
// Seed-nospread: stamp (tick, frac, pos) into the local CCSPlayer_WeaponServices
// shoot-position ring (+0xE8[32]) so GetInterpolatedShootPosition resolves the
// sub-tick shot and does NOT spam "[Shooting] cl: ... didn't find a shoot
// position history entry ...". Same 20B entry the engine writes. No-op on doubt.
bool StampShootPositionHistory(const C_CSPlayerPawn* pawn, int tick, float frac,
	const Vector_t& pos);
// Same stamp, but with the weapon-services pointer already resolved (used by the
// GetInterpolatedShootPosition hook, which receives it directly).
bool StampShootPositionHistorySvc(void* weaponServices, int tick, float frac,
	const Vector_t& pos);
// Hist nPlayerTickCount can lag ~10+ ticks behind the weaponServices ring
// (oldest..newest). Fire then looks up [ #stale + frac ] -> "[Shooting] cl:".
// If tick is older than the oldest ring entry, snap to newest (live tickbase).
...
// forceUpdate=false for Present/ESP/hitmarker; true for CreateMove combat.
...
// Map engine hitgroup -> Config::HB_* (-1 = unknown)
