...
struct FrameCapsuleCache {
	...
	// Per-studio-index capsules for RayHitsConfiguredHitbox - built once per
	// frame gen per index instead of per ray test (trigger/aim/HC call it
	// hundreds of times per tick; each build was a LookupBone vcall + quat
	// rotate + protected reads). Lifetime = one BeginFrame pass, same as byHb.
	Capsule byStudio[64]{};
	bool haveStudio[64]{};
	uintptr_t boneArr = 0;
	bool boneArrOk = false;
};
...
void Invalidate() {
	std::memset(g_cache, 0, sizeof(g_cache));
	g_cacheWrite = 0;
	std::memset(g_frameCache, 0, sizeof(g_frameCache));
	g_frameWrite = 0;
	g_frameGen = 0;
}
...
static bool ShootPosFromWeaponHistory(const C_CSPlayerPawn* pawn, Vector_t& out) {
	...
	// Newest slot = (head + count - 1) % 32 (head = oldest); scan back; max-tick fallback.
	Vector_t best{};
	int bestTick = -1;
	const int start = ((head + count - 1) % kSlots + kSlots) % kSlots;
...
// Seed-nospread fire stamp: the local engine resolves the bullet origin via
// GetInterpolatedShootPosition over the weaponServices shoot-position ring. The
// engine records whole-tick entries only; a seed fire stamped at (tick, frac)
// with frac > 0 has no matching entry -> "[Shooting] cl:" console spam from
// CSBaseGunFire when triggerbot/autofire seed-nospread fires. Write the exact
// sub-tick entry into the ring so the lookup succeeds (same 20-byte layout the
// engine itself appends: tick@0 frac@4 pos@8/12/16; head@0x368 count@0x36C).
namespace {
constexpr std::size_t kShootRingBase = 0xE8;
constexpr std::size_t kShootRingHead = 0x368;
constexpr std::size_t kShootRingCount = 0x36C;
constexpr int kShootRingSlots = 32;
constexpr int kShootRingEntry = 20;

bool ReadShootRingEnds(void* svc, int& oldTick, float& oldFrac,
	int& newTick, float& newFrac)
{
	oldTick = newTick = 0;
	oldFrac = newFrac = 0.f;
	if (!svc || !Mem::Valid(svc, 0x380))
		return false;
	int head = 0;
	int count = 0;
	if (!Mem::ReadField(svc, kShootRingHead, head))
		return false;
	if (!Mem::ReadField(svc, kShootRingCount, count))
		return false;
	head &= (kShootRingSlots - 1);
	if (count <= 0 || count > kShootRingSlots)
		return false;
	const auto* base = static_cast<const std::uint8_t*>(svc) + kShootRingBase;
	const auto* oldest = base + static_cast<std::size_t>(head) * kShootRingEntry;
	const auto* newest = base
		+ static_cast<std::size_t>((head + count - 1) & (kShootRingSlots - 1))
		* kShootRingEntry;
	if (!Mem::IsReadable(oldest, kShootRingEntry)
		|| !Mem::IsReadable(newest, kShootRingEntry))
		return false;
	std::memcpy(&oldTick, oldest + 0, 4);
	std::memcpy(&oldFrac, oldest + 4, 4);
	std::memcpy(&newTick, newest + 0, 4);
	std::memcpy(&newFrac, newest + 4, 4);
	return oldTick > 0 && newTick > 0;
}

void* WeaponServicesOf(const C_CSPlayerPawn* pawn)
{
	if (!Mem::ValidEntity(pawn))
		return nullptr;
	static uint32_t s_wpnSvcOff = 0;
	...
	void* svc = nullptr;
	__try {
		if (!Mem::ReadField(pawn, s_wpnSvcOff, svc))
			return nullptr;
	} __except (EXCEPTION_EXECUTE_HANDLER) {
		return nullptr;
	}
...
bool StampShootPositionHistorySvc(void* svc, int tick, float frac,
	const Vector_t& pos)
{
	if (!svc || !Mem::ValidEntity(svc) || tick <= 0)
		return false;
	if (!std::isfinite(frac) || frac < 0.f || frac >= 1.f)
		frac = 0.f;
	if (!IsValidPos(pos))
		return false;

	__try {
		if (!Mem::Valid(svc, 0x380))
			return false;

		// Ring layout (IDA GetInterpolatedShootPosition): 32 x 20B entries at
		// +0xE8. +0x368 = head = index of the OLDEST entry, +0x36C = count.
		// Entries run oldest->newest from slot head to slot (head+count-1)%32.
		// Appends go ONE PAST the newest: slot (head+count)%32, then count++.
		// Never append a tick older than newest - that disorders the ring and
		// the engine lookup still fails (spam: asked #8128, ring is #8139..#8141).
		int oldTick = 0, newTick = 0;
		float oldFrac = 0.f, newFrac = 0.f;
		if (ReadShootRingEnds(svc, oldTick, oldFrac, newTick, newFrac)) {
			const bool ahead = (tick > newTick)
				|| (tick == newTick && frac > newFrac + 1e-4f);
			if (!ahead)
				return true; // already covered / behind - do not reorder
		}
		...
		if (Mem::ReadField(svc, kShootRingHead, head))
			...
			return false;
		if (Mem::ReadField(svc, kShootRingCount, count))
			count = (count < kShootRingSlots) ? count : kShootRingSlots;
		else
			return false;

		const int slot = (head + count) & (kShootRingSlots - 1);
		auto* entry = reinterpret_cast<std::uint8_t*>(svc) + kShootRingBase
			+ static_cast<std::size_t>(slot) * kShootRingEntry;
		if (!Mem::IsReadable(entry, kShootRingEntry))
			return false;

		std::memcpy(entry + 0, &tick, 4);
		std::memcpy(entry + 4, &frac, 4);
		std::memcpy(entry + 8, &pos.x, 4);
		std::memcpy(entry + 12, &pos.y, 4);
		std::memcpy(entry + 16, &pos.z, 4);

		if (count < kShootRingSlots) {
			*reinterpret_cast<std::int32_t*>(
				reinterpret_cast<std::uint8_t*>(svc) + kShootRingCount) = count + 1;
		} else {
			*reinterpret_cast<std::int32_t*>(
				reinterpret_cast<std::uint8_t*>(svc) + kShootRingHead) =
				(head + 1) & (kShootRingSlots - 1);
		}
		return true;
	} __except (EXCEPTION_EXECUTE_HANDLER) {
		return false;
...
bool ClampSeedTickToShootRing(const C_CSPlayerPawn* pawn, int& tick, float& frac)
{
	if (tick <= 0)
		return false;
	void* svc = WeaponServicesOf(pawn);
	if (!svc)
		return false;
	int oldTick = 0, newTick = 0;
	float oldFrac = 0.f, newFrac = 0.f;
	if (!ReadShootRingEnds(svc, oldTick, oldFrac, newTick, newFrac))
		return false;
	const bool behind = (tick < oldTick)
		|| (tick == oldTick && std::isfinite(frac) && frac + 1e-4f < oldFrac);
	if (!behind)
		return false;
	// Snap to newest - that is tickbase / live fire. Solving on a 10+ tick
	// stale hist hash then looking it up in a 3-tick ring is the miss + spam.
	tick = newTick;
	frac = (std::isfinite(newFrac) && newFrac >= 0.f && newFrac < 1.f)
		? newFrac : 0.f;
	return true;
}

bool StampShootPositionHistory(
	const C_CSPlayerPawn* pawn,
	int tick,
	float frac,
	const Vector_t& pos)
{
	if (!Mem::ValidEntity(pawn) || tick <= 0)
		return false;
	void* svc = WeaponServicesOf(pawn);
	if (!svc)
		return false;
	return StampShootPositionHistorySvc(svc, tick, frac, pos);
}
...
bool GetSlotPos(uintptr_t boneArray, const Map& map, int slot, Vector_t& out) {
	...
	if (bi < 0)
		return false;
	if (map.boneCount > 0 && bi >= map.boneCount)
		return false;
	...
	float centerBias)
{
	...
	if (axisLen < 1e-4f) {
		...
		float sc, tc;
		if (denom < 1e-8f) {
			// Ray ? capsule axis: the line-line form degenerates - old code
			// forced sc=0 (measured at the eye) and rejected shots straight
			// down a distant limb/gun axis (enemy aiming at you). Clamp the
			// axis foot first, then project it back onto the ray: s = (t?b?d)/a.
			tc = std::clamp((c > 1e-8f) ? (e / c) : 0.f, 0.f, 1.f);
			sc = (a > 1e-8f) ? ((tc * b - d) / a) : 0.f;
		} else {
			sc = (b * e - c * d) / denom;
			tc = (a * e - b * d) / denom;
		}
	...
	// t >= 0 - old t < 1.f rejected face-hug range (< 1u to capsule surface)
	if (t < 0.f || t > 8192.f)
		return false;
	...
	float centerBias)
{
	...
	// Bone array - pointer cached per frame gen (the matrices behind it are
	// read fresh through the pointer; the pointer itself is stable per pass).
	uintptr_t boneArray = 0;
	Vector_t origin{};
	float height = 0.f;
	if (fc.boneArrOk) {
		boneArray = fc.boneArr;
	} else {
		if (!GetBoneArray(pawn, boneArray, origin, height))
			GetBoneArrayReadonly(pawn, boneArray, origin, height);
		fc.boneArr = boneArray;
		fc.boneArrOk = (boneArray != 0);
	}
