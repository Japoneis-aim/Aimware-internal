...
// body:
// if (!*a1) return; iface=*a1; vt=*iface; call vt[0x548]
...
memset(raw, 0, sizeof(raw));
		std::
memcpy(raw + 0x00, &mins, sizeof(float) * 3);
		std::
memcpy(raw + 0x0C, &maxs, sizeof(float) * 3);
		// Kind byte - engine's own RayT builder (IDA sub_18035F110, current
		// build): mins==maxs -> 0 (ray), else -> 2 = AABB (mins@+0x00,
		// maxs@+0x0C). Kind 1 is the SPHERE selector (radius@+0x0C) on this
		// build family - the old value 1 made every hull trace sweep a sphere
		// centered at `mins` with radius `maxs.x` (offset ?2 backwards), so
		// grenade hulls collided early on the min side and missed grazes on
		// the max side.
		raw[0x28] = 2;
	}
};
...
using InitFilterFn = void*(__fastcall*)(void* filter, void* skip, std::
uint64_t mask,
                                        int layer, int a5);
...
std::
atomic<int>  g_avStreak{0};
std::
atomic<bool> g_traceDisabled{false};
std::
atomic<int>  g_okStreak{0};
...
// constexpr std::
uint32_t kOwnerEntityOff = 0x520;
...
constexpr std::
size_t kTraceShapeVtableOff = 0x548;

std::
uint8_t* FindClient(const char* pat) {
	auto* p = M::
FindPattern("client.dll", pat);
	if (!p)
		p = M::
FindPattern("client", pat);
	return p;
}

bool VtableLooksGood(void* iface) {
	if (!iface || !Mem::
IsReadable(iface, sizeof(void*)))
		return false;
	void* vt = nullptr;
	__try {
...
IsUserPtr(vt))
		return false;
	if (!Mem::
IsReadable(reinterpret_cast<std::
uint8_t*>(vt) + kTraceShapeVtableOff, sizeof(void*)))
		return false;
	void* method = nullptr;
	__try {
		method = *reinterpret_cast<void**>(reinterpret_cast<std::
uint8_t*>(vt) + kTraceShapeVtableOff);
	} __except (EXCEPTION_EXECUTE_HANDLER) {
		return false;
	}
	return method && Mem::
...
void* ResolveWorldArg() {
	if (!g_ppTraceMgr || !Mem::
IsReadable(g_ppTraceMgr, sizeof(void*)))
		return nullptr;

	void* world = *g_ppTraceMgr; // same value game loads into rcx
	if (!world || !Mem::
IsUserPtr(world) || !Mem::
IsReadable(world, sizeof(void*)))
		return nullptr;
...
IsReadable(world, sizeof(void*))) {
			Con::
Warn("TraceShape: world ptr unreadable %p", world);
			return false;
		}
		void* iface = *reinterpret_cast<void**>(world);
		if (!iface || !Mem::
IsUserPtr(iface)) {
			Con::
Warn("TraceShape: iface invalid %p (world=%p)", iface, world);
			return false;
		}
		// DidHit return is not success - call surviving without AV is success
		(void)
g_traceShape(world, ray, start, end, filter, out);
		NoteTraceOk();
		return true;
	} __except (EXCEPTION_EXECUTE_HANDLER) {
...
void* SehInitFilter(void* filter, void* skip, std::
uint64_t mask, int layer, int a5) {
	__try {
...
// +0x38 layer byte
// +0x40 multi-hit = 0
void NormalizeFilter(std::
uint8_t* filter, std::
uint64_t mask) {
	if (!filter)
		return;
	*reinterpret_cast<std::
uint64_t*>(filter + 8) = mask;
	// FireBullet: v25=-65536 @+0x32, v26=3840 @+0x36
	*reinterpret_cast<std::
int32_t*>(filter + 0x32) = -65536;
	*reinterpret_cast<std::
int16_t*>(filter + 0x36) = 3840;
...
memory_order_relaxed)) {
		static std::
atomic<int> s_probe{0};
		if ((s_probe.fetch_add(1) % 16) != 0)
			return false;
	}
	if (!g_traceShape || !Mem::
IsUserPtr(reinterpret_cast<void*>(g_traceShape)))
		return false;
	if (!g_initFilter || !Mem::
IsUserPtr(reinterpret_cast<void*>(g_initFilter)))
		return false;
...
ValidEntity(cur); ++depth) {
		...
		__try {
			if (asBase->IsBasePlayer() && static_cast<void*>(asBase) == target)
				return true;
		} __except (EXCEPTION_EXECUTE_HANDLER) { TW_SEH_CATCH("trace.isBasePlayer"); }

		CBaseHandle owner{};
		__try {
			owner = *reinterpret_cast<CBaseHandle*>(
				reinterpret_cast<std::
uint8_t*>(cur) + Offset::m_hOwnerEntity());
		} __except (EXCEPTION_EXECUTE_HANDLER) {
			break;
		}
		if (!owner.valid() || !I::
...
bool Init() {
	...
	const uintptr_t mgrSite = M::
patternScan("client", kPatGameTraceMgr);
	if (mgrSite)
		g_ppTraceMgr = reinterpret_cast<void**>(M::
getAbsoluteAddress(mgrSite, 3));
	if (!g_ppTraceMgr) {
		const uintptr_t mgrLoose = M::
patternScan("client", kPatGameTraceMgrLoose);
		if (mgrLoose)
			g_ppTraceMgr = reinterpret_cast<void**>(M::
getAbsoluteAddress(mgrLoose, 3));
	}
	if (!g_ppTraceMgr) {
		const uintptr_t physSite = M::
patternScan("client", kPatPhysWorld);
		if (physSite)
			g_ppTraceMgr = reinterpret_cast<void**>(M::
getAbsoluteAddress(physSite, 3));
	}

	void* slot = (g_ppTraceMgr && Mem::
IsReadable(g_ppTraceMgr, sizeof(void*))) ? *g_ppTraceMgr : nullptr;
	void* world = ResolveWorldArg();
	void* iface = (world && Mem::
IsReadable(world, sizeof(void*))) ? *reinterpret_cast<void**>(world) : nullptr;

	Con::
Info("Trace: shape=%p filter=%p global=%p slot=%p world=%p iface=%p",
		(void*)
g_traceShape, (void*)
g_initFilter, (void*)
g_ppTraceMgr, slot, world, iface);

	if (!g_traceShape) Con::PatternMiss("client", "Trace::TraceShape");
	if (!g_initFilter) Con::PatternMiss("client", "Trace::InitFilter");
	if (!g_ppTraceMgr) Con::PatternMiss("client", "Trace::GameTraceManager");

	g_ready = g_traceShape && g_initFilter && g_ppTraceMgr;
	if (g_ready)
		Con::
Ok("Trace system ready (pass *global, mask rewrite on)");
	else
		Con::
Error("Trace system incomplete");
	return g_ready;
}

bool Ready() {
	return g_ready && !g_traceDisabled.load(std::
memory_order_relaxed);
}
...
ValidEntity(skip))
	...
	if (g_initTraceInfo && Mem::
IsUserPtr(reinterpret_cast<void*>(g_initTraceInfo)))
...
uint16_t a5) {
	std::
memset(out.raw, 0, sizeof(out.raw));
	// Full finite validation - a NaN from a stale bone read must never reach
	// the engine phys vcall (6 AVs trip the soft-disable threshold and kill
	// the whole trace system). TraceLine validates all axes; match it here.
	if (!Mem::
Finite(start.x) || !Mem::
Finite(start.y) || !Mem::
Finite(start.z)
...
uint64_t mask, bool penetrate) {
	if (!ends || count <= 0 || count > 64)
		return false;
	if (!Mem::
IsReadable(ends, static_cast<std::
size_t>(count) * sizeof(Vector_t)))
		return false;
...
bool IsBodyVisible(const Vector_t& eye, const Vector_t* bones, int boneCount,
                   void* skip, void* target, std::
uint64_t mask) {
	if (!bones || boneCount <= 0 || boneCount > 64 || !Mem::
ValidEntity(target))
		return false;
...
isfinite(f[3]))
			outHardness = f[3];
		return std::
isfinite(outPenetration) || std::
isfinite(outHardness);
	} __except (EXCEPTION_EXECUTE_HANDLER) {
		return false;
	}
}

bool GetHitElasticity(const CGameTrace& tr, float& outElasticity) {
	outElasticity = 1.f;
	if (!g_getSurfaceData)
		return false;
	void* surf = tr.surface();
	...
	if (!data)
		return false;
	auto readElas = [](const void* p, std::uint32_t off, float& o) -> bool {
		if (!p || !Mem::IsReadable(p, off + 4))
			return false;
		float e = 0.f;
		__try { e = *reinterpret_cast<const float*>(static_cast<const std::uint8_t*>(p) + off); }
		__except (EXCEPTION_EXECUTE_HANDLER) { return false; }
		if (!std::isfinite(e) || e < 0.f || e > 2.f)
			return false;
		o = e;
		return true;
	};
	// Full CPhysSurfaceProperties: m_physics @ 0x28, m_elasticity @ +4
	if (readElas(data, 0x2C, outElasticity))
		return true;
	// Physics block only
	if (readElas(data, 0x4, outElasticity))
		return true;
	return false;
}

bool GetHitFriction(const CGameTrace& tr, float& outFriction) {
	outFriction = 1.f;
	if (!g_getSurfaceData)
		return false;
	void* surf = tr.surface();
	...
	if (!data)
		return false;
	auto readFric = [](const void* p, std::uint32_t off, float& o) -> bool {
		if (!p || !Mem::IsReadable(p, off + 4))
			return false;
		float e = 0.f;
		__try { e = *reinterpret_cast<const float*>(static_cast<const std::uint8_t*>(p) + off); }
	...
	// Full CPhysSurfaceProperties: m_physics @ 0x28, m_friction @ +0
	return readFric(data, 0x28, outFriction);
}
