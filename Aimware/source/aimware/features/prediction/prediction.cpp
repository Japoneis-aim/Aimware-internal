...
{
	// IDA 2026-08-25: gpGlobals slot = off_1820B05F0 (1140 refs, curtime @ +0x30).
	...
	// pGlobalVariables: mov [rip+slot], rdx inside the clock-swap fn
	hit = M::FindPattern("client", "48 89 15 ? ? ? ? 48 89 42");
	if (hit) {
		void** pp = reinterpret_cast<void**>(M::GetAbsoluteAddress(hit, 3, 0));
		if (void* gv = SehReadPtr(pp))
			return gv;
	}
	// Live-slot RVA fallback (old 0x2090D60 dead on current build)
...
bool BackupPawn(C_CSPlayerPawn* pawn, PawnBackup& out)
{
	...
	__try {
		...
		if (hCtrl.valid() && I::GameEntity && I::GameEntity->Instance) {
			...
			if (ctrl) {
				out.controller = ctrl;
				// GetTickBase 0x902560 was breaking bhop - revert to schema m_nTickBase (stable)
				// Direct call matched wrong function (call-site pattern hit wrong E8). Keep schema.
...
bool Init()
{
	...
	// IDA pPrediction (client.dll.i64 919f9a64):
	// hit 0x180B91AB0: lea rax,[unk_1823A5140]; ret -> returns &CPrediction
	// pattern unique (1 match). Dump json rva 0x23A9020 is stale.
	// Fallback RVA 0x23A5140 only if pattern miss.
	...
	// ProcessMovement 0x180A1B5C0 - unique prologue
	...
	// RunCommand_Context 0x180A1CF90 (resolved for consumers; Start must NOT call it)
	...
	// ProcessSubTickInput 0x180B04240 - 3 args (this, slot, ctx). Not called from Start.
	...
	// QueueForceSubtickMove / ProcessForceSubtickMoves / SetupMove - DEAD.
	// IDA: dump Queue/Process land on schema registrars (CPlayer_MovementServices
	// field table incl. m_arrForceSubtickMoveWhen). SetupMove dump pattern is
	// unique but is buywheel UI (sub_180D55D10, "buywheel-cant-afford").
	g_eng.queueForceSubtickMove = nullptr;
	g_eng.processForceSubtickMoves = nullptr;
	g_eng.setupMove = nullptr;

	g_eng.globalVars = ResolveGlobalVars();

	// GetTickBase - direct call instead of schema read (dump 0x902560)
	// Pattern from patterns.json: E8 ? ? ? ? EB ? 48 8B 05 ? ? ? ? 8B 40
	// bytes 8B 81 B8 06 00 00 = mov eax,[rcx+0x6B8] (controller tickbase)
	g_eng.getTickBase = M::FindPattern("client", "E8 ? ? ? ? EB ? 48 8B 05 ? ? ? ? 8B 40");
	if (!g_eng.getTickBase) {
		// Fallback synth from bytes
		g_eng.getTickBase = M::FindPattern("client", "8B 81 B8 06 00 00 C3");
		if (g_eng.getTickBase)
			Con::Ok("Pred: GetTickBase fallback @ %p", g_eng.getTickBase);
	} else {
		// Resolve rip call to actual GetTickBase body
		if (auto* call = M::FindPattern("client", "E8 ? ? ? ? EB ? 48 8B 05 ? ? ? ? 8B 40")) {
			g_eng.getTickBase = ResolveRipCall(call);
			Con::Ok("Pred: GetTickBase @ %p (resolved via call)", g_eng.getTickBase);
		}
	}
	if (!g_eng.getTickBase) Con::OffsetMiss("Pred::GetTickBase (will use schema fallback)");

	// Patterns resolved for InputInject / StampInputHistory / optional sim.
	// Pred::Start must NOT call RunCommand_Context - extra slot -> full repredict.
	...
	else Con::Ok("Pred: CPrediction @ %p (in_pred@+0x34 first@+0xF0 WORD)", g_eng.pPrediction);
	...
	// SetupMove / force-subtick intentionally NOT resolved - dump names are
	// schema registrars or buywheel UI, not movement SetupMove.
	if (g_eng.runCommand)
		Con::Ok("Pred: RunCommand_Context @ %p (canonical engine sim path)",
			g_eng.runCommand);
	else
		Con::OffsetMiss("Pred::RunCommand_Context (resolved for consumers; Start does not call it)");
...
{
	// Original CreateMove already predicted this cmd into the client slot ring.
	// A second RunCommand / SetPredCmd / tickbase++ records another slot:
	// "server acknowledged to slot 4 client only predicted into 3 slots,
	// full repredict will occur"
	// That full repredict is the online dsync (seed tick+1 miss, land rubberband).
	// Never call RunCommand - snapshot live post-CM state only.
	...
	PawnBackup liveBak{};
	const bool haveBak = BackupPawn(pawn, liveBak);

	PredictedState captured{};
	if (haveBak && CapturePredicted(pawn, moveSvc, liveBak, captured, false)) {
		captured.fromEngine = false;
		captured.usedRunCommand = false;
		g_last = captured;
	} else {
		LocalFallbackPredict(pawn, cmd, g_last, haveBak ? &liveBak : nullptr);
	}
...
// ??_opt = arccos((wishspeed - accelspeed) / |v|) where accelspeed = accel*wishspeed*dt.
