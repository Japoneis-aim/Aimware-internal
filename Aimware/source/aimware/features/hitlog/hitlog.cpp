...
float Now()
{
	return static_cast<float>(ImGui::
GetTime());
}
...
{
	const int aa = static_cast<int>(((c >> IM_COL32_A_SHIFT) & 0xFF) * std::
clamp(a, 0.f, 1.f));
...
memcpy(tmp, buf, len);
		...
		tmp[len + 3] = '\0';
		if (ImGui::
CalcTextSize(tmp).x <= maxW) {
			std::
memcpy(buf, tmp, len + 4);
			return;
		}
		buf[len - 1] = '\0';
	}
...
GetIO().DisplaySize;
	pos.x = std::
clamp(pos.x, 4.f, (std::
max)(4.f, ds.x - panelW - 4.f));
	pos.y = std::
clamp(pos.y, 4.f, (std::
max)(4.f, ds.y - panelH - 4.f));
	return pos;
}
...
snprintf(e.name, sizeof(e.name), "%s", name);
		...
		std::
snprintf(e.name, sizeof(e.name), "player");
	...
	if (kill)
		++g_sessKills;
	char nameCopy[64]{};
	std::snprintf(nameCopy, sizeof(nameCopy), "%s", e.name);
	int hlCopy = e.healthLeft;
	ReleaseSRWLockExclusive(&g_lock);
...
hitlog_console) {
		...
		// Channel tag = (Aimware) via Notify brand log.
		// Body: HIT name -DMG TAG ...
		if (kill)
			Notify::
ConsoleColorPrintf(cr, cg, cb, 255,
				"HIT  %s  -%d  %s  | KILL", nameCopy, damage, hg);
		else if (hlCopy >= 0)
			Notify::
ConsoleColorPrintf(cr, cg, cb, 255,
				"HIT  %s  -%d  %s  | %dhp left", nameCopy, damage, hg, hlCopy);
		else
			Notify::
ConsoleColorPrintf(cr, cg, cb, 255,
				"HIT  %s  -%d  %s", nameCopy, damage, hg);
	}
}
...
hitlog)
	...
	AcquireSRWLockExclusive(&g_lock);
	const float life = std::
clamp(Config::
hitlog_duration, 1.f, 12.f);
	const float now = Now();
	const ImGuiIO& io = ImGui::
GetIO();
	...
	const int maxVis = std::
clamp(Config::
hitlog_max_rows, 4, kMax);
	...
	for (int n = 0; n < kMax; ++n) {
		...
		row.e = &e;
		row.age = age;
		const float enterT = EaseOut(std::
clamp(age / slideIn, 0.f, 1.f));
		row.slide = enterT;
		float a = 1.f;
		if (age > life - fadeOut) {
...
clamp(a, 0.f, 1.f);
	...
	if (nRows <= 0 && !menuOpen) { ReleaseSRWLockExclusive(&g_lock); return; }
...
GetFontSize();
	...
	const float statsH = Config::
hitlog_show_stats ? (fs + 8.f) : 0.f;
	const float panelW = std::
clamp(Config::
hitlog_width, 200.f, 420.f);
	const int drawRows = (nRows > 0) ? nRows : (menuOpen ? 1 : 0);
	const float bodyH = (drawRows > 0)
		? (float)
drawRows * rowH + (float)(drawRows - 1) * rowGap
		: 0.f;
...
PushStyleColor(ImGuiCol_WindowBg, ImVec4(0, 0, 0, 0));
	...
	ImGui::
Begin("##hitlog_panel", nullptr, flags);
	{
		ImGui::
InvisibleButton("##hl_hit", ImVec2(panelW, panelH));
		const bool hovered = ImGui::
IsItemHovered();
...
IsMouseDragging(ImGuiMouseButton_Left)) {
				Config::
hitlog_pos.x += io.MouseDelta.x;
				Config::
hitlog_pos.y += io.MouseDelta.y;
				Config::
hitlog_pos = ClampPos(Config::
hitlog_pos, panelW, panelH);
				ImGui::
SetWindowPos(Config::
hitlog_pos);
			} else {
...
hitlog_pos;
				Config::
hitlog_pos = ClampPos(p, panelW, panelH);
				if (Config::
hitlog_pos.x != p.x || Config::
hitlog_pos.y != p.y)
					ImGui::
SetWindowPos(Config::
hitlog_pos);
...
CalcTextSize(tip);
			...
			char cnt[16];
			std::
snprintf(cnt, sizeof(cnt), "%d", g_sessHits);
			const ImVec2 csz = ImGui::
CalcTextSize(cnt);
...
CalcTextSize(tag);
			...
			char dmgBuf[16];
			std::
snprintf(dmgBuf, sizeof(dmgBuf), "-%d", e.damage);
			const ImVec2 dmgSz = ImGui::
CalcTextSize(dmgBuf);
			...
			ImVec2 hpSz{};
			if (Config::
hitlog_show_hp && e.healthLeft >= 0 && !e.kill) {
				std::
snprintf(hpBuf, sizeof(hpBuf), "%d", e.healthLeft);
				hpSz = ImGui::
CalcTextSize(hpBuf);
				rightX -= (hpSz.x + 8.f);
			}
			...
			char nameLine[72];
			std::
snprintf(nameLine, sizeof(nameLine), "%s", e.name);
			const float nameX = tagX + tagSz.x + 8.f;
			const float nameMaxW = (std::
max)(0.f, rightX - nameX - 8.f);
			ClipName(nameLine, sizeof(nameLine), nameMaxW);
			const ImVec2 nsz = ImGui::
CalcTextSize(nameLine);
			dl->AddText(
			...
			// Life underline
			const float lifeFrac = 1.f - std::
clamp(row.age / life, 0.f, 1.f);
		...
		if (Config::
hitlog_show_stats && (g_sessHits > 0 || menuOpen)) {
			...
			char foot[96];
			std::
snprintf(foot, sizeof(foot), "H %d  HS %d  K %d  DMG %d",
				g_sessHits, g_sessHeads, g_sessKills, g_sessDmg);
			const ImVec2 fsz = ImGui::
CalcTextSize(foot);
			dl->AddText(
		...
		if (menuOpen && hovered && ImGui::
IsMouseClicked(ImGuiMouseButton_Right))
			ImGui::
OpenPopup("##hitlog_widget_settings");

		if (ImGui::
BeginPopup("##hitlog_widget_settings")) {
			ImGui::
TextUnformatted("HIT LOG");
			ImGui::
Separator();
			ImGui::
Checkbox("Show HP left", &Config::
hitlog_show_hp);
			ImGui::
Checkbox("Session stats", &Config::
hitlog_show_stats);
			ImGui::
Checkbox("Game console", &Config::
hitlog_console);
			ImGui::
SetNextItemWidth(160.f);
			ImGui::
SliderFloat("Duration", &Config::
hitlog_duration, 1.f, 12.f, "%.1f s");
			ImGui::
SetNextItemWidth(160.f);
			ImGui::
SliderFloat("Width", &Config::
hitlog_width, 200.f, 380.f, "%.0f");
			ImGui::
SetNextItemWidth(160.f);
			ImGui::
SliderInt("Max rows", &Config::
hitlog_max_rows, 4, 16);
			ImGui::
ColorEdit4("Body", (float*)&Config::
hitlog_color, ImGuiColorEditFlags_NoInputs);
			ImGui::
SameLine();
			ImGui::
ColorEdit4("Head", (float*)&Config::
hitlog_head_color, ImGuiColorEditFlags_NoInputs);
			ImGui::
SameLine();
			ImGui::
ColorEdit4("Kill", (float*)&Config::
hitlog_kill_color, ImGuiColorEditFlags_NoInputs);
			ImGui::
Dummy(ImVec2(0, 4.f));
			if (ImGui::
Button("Reset position", ImVec2(-1.f, 0.f)))
				Config::
hitlog_pos = ImVec2(-1.f, -1.f);
			if (ImGui::
Button("Clear log", ImVec2(-1.f, 0.f))) {
				// inline clear while already holding g_lock (avoid deadlock on Clear())
				for (int ci = 0; ci < kMax; ++ci) g_log[ci] = Entry{};
				g_write = 0;
				g_sessHits = 0;
				g_sessHeads = 0;
				g_sessKills = 0;
