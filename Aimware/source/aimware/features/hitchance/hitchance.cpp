...
// IDA FillGunFireData / CSBaseGunFire fill @ 0x1807D1180 (live, was 0x1807CFDD0)
...
// Old 160-320 x capsule tests caused CreateMove FPS spikes with trigger HC.
	...
	C_CSPlayerPawn* target)
{
	...
	// CS2: bullet dir = hist view + GetRemovedAimPunch. Callers pass unpunched hist.
	...
	// Budget cap: total ray-capsule tests per call ? nSamples x nBullets.
	// Shotgun (16 pellets) at full MC depth = 2048 tests per call per tick -
	// scale the sample count instead (statistical accuracy stays sufficient).
	int nSamples = McSampleCount(requiredPct, bloom);
	if (nBullets > 1)
		nSamples = (std::max)(8, (std::min)(nSamples, 512 / nBullets));
	...
	bool useLocalSpread)
{
	...
	std::uint32_t seed = 0;
	float sx = 0.f, sy = 0.f;
	// useLocalSpread=true: search path (NoSpread::Solve 20-80x/tick).
	...
	int maxDirs)
{
	...
	// ExactShotHits is the FIRE-time verify (called ~1x per fire attempt from
	...
	// GetBulletDirectionCached (called 20-80x per tick, would stomp too much).
