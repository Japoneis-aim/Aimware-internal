...
uintptr_t Scan(const char* tag, const char* pat) {
	uintptr_t a = M::
patternScan("engine2", pat);
	if (a)
		Con::
Ok("Engine2 %s @ 0x%p", tag, (void*)
a);
	else
		Con::PatternMiss("engine2", tag);
	return a;
}
...
const char* SehMapAt(void* ngc, std::
uintptr_t off) {
	if (!ngc || !Mem::
IsReadable(ngc, off + 8))
		return nullptr;
	__try {
		const char* p = *reinterpret_cast<const char**>(reinterpret_cast<std::
uint8_t*>(ngc) + off);
		if (!p || !Mem::
IsReadable(p, 2) || !p[0])
			return nullptr;
