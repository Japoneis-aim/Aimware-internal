...
using FnEvtGetInt64 = std::
int64_t(__fastcall*)(void*, const char*, unsigned int);
// engine2 ExecuteStringCommand(this, cmd)
using FnExecStr = void(__fastcall*)(void* thisptr,
const char* cmd);
...
std::
atomic<ULONGLONG> g_autoAt{ 0 };
std::
atomic<int> g_autoChoice{ 0 }; // 0 yes, 1 no
std::
atomic<bool> g_autoPending{ false };
std::
atomic<ULONGLONG> g_lastRevealMs{ 0 };
std::
atomic<int> g_lastVoterKey{ -1 };
...
std::
uint32_t Murmur2Token(const char* name) {
...
uint32_t m = 0x5bd1e995u;
	const int r = 24;
	std::
uint32_t h = 0x31415926u ^ static_cast<std::
uint32_t>(len);
...
vote_auto)
	...
	const ULONGLONG delay = static_cast<ULONGLONG>(
		std::
clamp(Config::
vote_auto_delay_ms, 0.f, 5000.f));
	g_autoChoice.store(Config::
vote_auto_choice == 1 ? 1 : 0, std::
memory_order_relaxed);
	g_autoAt.store(GetTickCount64() + delay, std::
memory_order_relaxed);
	g_autoPending.store(true, std::
memory_order_relaxed);
}
...
EngineClient)
		return;
	const int choice = g_autoChoice.load(std::
memory_order_relaxed);
...
EngineClient, cmd);
	Notify::
Push("Auto Vote", choice == 1 ? "Voted NO" : "Voted YES", Notify::
Type::
Success, 2.5f);
}
...
vote_reveal)
	...
	const int key = static_cast<int>(reinterpret_cast<std::
uintptr_t>(voter) ^ static_cast<unsigned>(opt));
	const ULONGLONG now = GetTickCount64();
	if (g_lastVoterKey.load(std::
memory_order_relaxed) == key
		&& now - g_lastRevealMs.load(std::
memory_order_relaxed) < 80ull)
		return;
	g_lastVoterKey.store(key, std::
memory_order_relaxed);
	g_lastRevealMs.store(now, std::
memory_order_relaxed);

	char body[96]{};
	std::
snprintf(body, sizeof(body), "%s -> %s", name, OptionLabel(opt));
	Notify::
Push("Vote", body, Notify::
Type::
Info, 3.5f);
	Notify::
ChatPrintf("\x04[Vote]\x01 %s voted \x0B%s", name, OptionLabel(opt));
	// Developer console (~) - always with vote reveal
	Notify::
ConsolePrintf("[Vote] %s voted %s", name, OptionLabel(opt));
}

void HandleVoteChanged(void* gameEvent) {
	// New vote activity -> arm auto-vote once
	if (Config::
vote_auto && !g_autoPending.load(std::
memory_order_relaxed))
		ScheduleAutoVote();

	if (!Config::
vote_reveal || !g_getInt64)
		return;
...
s_lastYes = yes;
	...
	Notify::
ConsolePrintf("[Vote] tally Yes %d  No %d",
...
patternScan(XS("client"),
	...
	// engine2 ExecuteStringCommand - (this, cmd)
	if (const uintptr_t pExec = M::
patternScan(XS("engine2"),
		XS("40 53 56 48 81 EC 48 07 00 00 48 8B F1 48 8B DA"))) {
		g_execStr = reinterpret_cast<FnExecStr>(pExec);
		Con::
Ok("Vote ExecuteStringCommand @ 0x%p", (void*)
pExec);
	} else {
		Con::
OffsetMiss("Vote ExecuteStringCommand");
	}

	g_ready = g_getName && g_getController && g_getInt64;
	if (g_ready)
		Con::
Ok("Vote event helpers ready");
	else
		Con::
Error("Vote event helpers incomplete");
}

void OnGameEvent(void* gameEvent) {
	if ((!Config::
vote_reveal && !Config::
vote_auto) || !gameEvent || !g_ready || !g_getName)
		return;
...
memory_order_relaxed))
	...
	if (std::
strcmp(name, "vote_changed") == 0) {
		HandleVoteChanged(gameEvent);
		return;
	}
	// Round / map wipe
	if (std::
strcmp(name, "round_start") == 0 || std::
strcmp(name, "game_newmap") == 0) {
		g_autoPending.store(false, std::
memory_order_relaxed);
	}
}
