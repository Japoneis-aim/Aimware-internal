...
scope_zoom_fov)
	...
	if (zl == 1)
		outFov = std::
clamp(Config::
scope_fov_1, 1.f, 90.f);
	else
		outFov = std::
clamp(Config::
scope_fov_2, 1.f, 90.f);
	return true;
}
...
std::
int64_t __fastcall H::
hkDrawScopeOverlay(void* a1, void* a2) {
	auto original = H::
DrawScopeOverlay.GetOriginal();
	std::
int64_t result = 0;
	if (original) {
	...
	if (!Config::
scope_custom_lines || !Scope::
LocalScopedForOverlay())
		return result;

	__try {
		auto* out = reinterpret_cast<std::
uint8_t*>(a2);
		out[4] = 0; // draw = false - kill bars/texture/stock lines
	} __except (EXCEPTION_EXECUTE_HANDLER) { TW_SEH_CATCH("scope.disableStockOverlay"); }
...
GetIO().DisplaySize;
	...
	const float size = std::
clamp(Config::
scope_line_size, 0.f, 1.f);
	const float gap = std::
clamp(Config::
scope_line_gap, 0.f, 80.f);
	// Hairline range: 0.1 .. 6 (was min 0.5 + heavy outline = always thick)
	const float th = std::
clamp(Config::
scope_line_thickness, 0.1f, 6.f);

	const float maxArmX = (std::
max)(0.f, cx - gap);
	const float maxArmY = (std::
max)(0.f, cy - gap);
	...
	const ImVec4& c = Config::
scope_line_color;
	const int a = static_cast<int>(std::
clamp(c.w, 0.f, 1.f) * 255.f);
	if (a < 8)
		return;
	const ImU32 col = IM_COL32(
		static_cast<int>(std::
clamp(c.x, 0.f, 1.f) * 255.f),
		static_cast<int>(std::
clamp(c.y, 0.f, 1.f) * 255.f),
		static_cast<int>(std::
clamp(c.z, 0.f, 1.f) * 255.f),
		a);
