...
ImTextureID Get(std::uint64_t steamId, ID3D11Device* device) {
	...
	// Bound the cache: spectators cycle, entries never evicted otherwise, and
	// every SRV is a live D3D11 allocation for the whole session. Full wipe at
	// the cap - entries rebuild through the 1.5 s backoff-limited retry path.
	if (g_cache.size() >= 64 && g_cache.find(steamId) == g_cache.end()) {
		for (auto& kv : g_cache) {
	...
	// Texture creation failed - back off like the other failure paths or this
	// becomes a per-frame RGBA download + CreateTexture2D churn loop.
	slot.nextTryMs = now + 5000;
	return ImTextureID_Invalid;
}
