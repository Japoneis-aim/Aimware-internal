...
void SetHeld(CUserCmd* cmd, std::
uint64_t button, bool down);
void SetEdge(CUserCmd* cmd, std::
...
void Clear(CUserCmd* cmd, C_CSPlayerPawn* pawn, std::
uint64_t button);

// Move-services ForceButtonsDown (ProcessMovement samples this).
bool ForceDown(C_CSPlayerPawn* pawn, std::
uint64_t buttons);

// Wipe ALL engine subtick steps before rewrite.
// Held-space / key-event garbage left in the list = online land desync.
...
// Wipe stale IN_ATTACK / IN_SECOND_ATTACK subtick steps (semi double-fire).
void ClearAttackSubticks(CBaseUserCmdPB* base);
...
// WASD strafe yaw step - quantized-movement yaw delta @ when.
bool SubtickYawDelta(CBaseUserCmdPB* base, float when, float yawDelta);

// Analog fix: when no feature wrote a move
// subtick step, append one carrying base-move vs last-processed-move deltas.
