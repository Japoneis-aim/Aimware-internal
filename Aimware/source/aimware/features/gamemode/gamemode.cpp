...
bool AreTeammatesEnemies() {
	...
	__try {
		...
		if (pData < 0x10000)
			return false;
		// IDA sub_181862B30: bool lives at data+0x58. Only 0/1 is valid -
		// garbage here used to mark competitive as FFA.
		const uint8_t v = *reinterpret_cast<uint8_t*>(pData + kCvarValueOffset);
		g_cvarCached = (v == 1);
...
void ResolveGameTypes() {
	...
	// IDA 0x180720580: Deathmatch check -
	//   !GetGameType() && GetGameMode()==2  via qword_182315EF8
	//   vfunc +152 / +160 = idx 19 / 20.
	// Old GetGameModeName pattern resolved a different iface.
	auto* p = M::FindPattern("client.dll",
		"48 8B 0D ? ? ? ? 48 8B 01 FF 90 98 00 00 00 85 C0 75 ? 48 8B 0D");
	if (!p)
		p = M::FindPattern("client",
			"48 8B 0D ? ? ? ? 48 8B 01 FF 90 98 00 00 00 85 C0 75 ? 48 8B 0D");
	if (!p) {
		p = M::FindPattern("client.dll",
			"48 8B 0D ? ? ? ? 48 8B 01 FF 90 98 00 00 00");
		if (!p)
			p = M::FindPattern("client",
				"48 8B 0D ? ? ? ? 48 8B 01 FF 90 98 00 00 00");
	}
	...
	auto* global = M::GetAbsoluteAddress(p, 3);
	if (!global)
		return;
...
bool SingleTeamLobby() {
	...
	int teamsMask = 0;
	int players = 0;
	const int lim = (nMax < 128) ? nMax : 128; // Controllers live in low slots 1..128
...
void Scan() {
	...
	// 1) Authoritative: convar + IGameTypes
	if (AreTeammatesEnemies()) {
		SetFfa("FFA/cvar");
		return;
	}
...
void SetMapCached(const char* mapName) {
	...
	if (!_stricmp(base, "<empty>") || !_stricmp(base, "empty")
		|| !_stricmp(base, "none") || !_stricmp(base, "null") || !_stricmp(base, "unknown"))
		return;

	// Workshop bare "mirage" -> "de_mirage" so official + workshop share lineups file
	if (!_stricmp(base, "mirage")) base = "de_mirage";
	else if (!_stricmp(base, "dust2") || !_stricmp(base, "dust") || !_stricmp(base, "dust_2")) base = "de_dust2";
	else if (!_stricmp(base, "inferno")) base = "de_inferno";
	else if (!_stricmp(base, "overpass")) base = "de_overpass";
	else if (!_stricmp(base, "nuke")) base = "de_nuke";
	else if (!_stricmp(base, "vertigo")) base = "de_vertigo";
	else if (!_stricmp(base, "ancient")) base = "de_ancient";
	else if (!_stricmp(base, "anubis")) base = "de_anubis";
	else if (!_stricmp(base, "train")) base = "de_train";
	else if (!_stricmp(base, "cache")) base = "de_cache";
	else if (!_stricmp(base, "office")) base = "cs_office";
	else if (!_stricmp(base, "italy")) base = "cs_italy";
...
bool EnsureMap() {
	...
	if (g_map[0] && !LooksLikeMapName(g_map))
		g_map[0] = '\0';
	if (g_map[0] && LooksLikeMapName(g_map)) {
		// Cached map answers this call - but it MUST be re-verified against
		// the engine on a slow lane, or the first map of the session sticks
		// forever (mirage -> dust2 still showed mirage lineups). Placeholder
		// reads during the load screen are rejected by SetMapCached, so the
		// cache only moves when the engine names a different real map.
		static std::uint64_t s_lastVerifyMs = 0;
		const std::uint64_t now = GetTickCount64();
		if (now - s_lastVerifyMs >= 2000ull) {
			s_lastVerifyMs = now;
			...
			// (localized TryClientMapName deliberately skipped here - display
			// name must never overwrite a real cache)
			if (raw) {
				char want[128]{};
				std::snprintf(want, sizeof(want), "%s", g_map);
				SetMapCached(raw); // no-op for placeholders / garbage
				if (_stricmp(want, g_map) != 0)
					Con::Ok("GameMode map changed: %s -> %s", want, g_map);
			}
		}
		return true;
	}
...
bool WantTeamCheck(bool userPref) {
	Tick();
	const bool pref = userPref || Config::team_check || Config::teamCheck;
	if (!pref)
		return false;
	if (AreTeammatesEnemies())
		return false;
