...
// C_CSPlayerResource->m_bombsiteCenterA/B dump fallbacks (SiteOffA/B resolve schema-first)
...
uintptr_t ScanClient(const char* tag, const char* pat) {
	uintptr_t a = M::
patternScan("client", pat);
	if (!a)
		a = M::
patternScan("client.dll", pat);
	if (a)
		Con::
Ok("Bomb %s @ 0x%p", tag, (void*)
a);
	else
		Con::PatternMiss("client", tag);
	return a;
}
...
fabs(v.z) < 1.f)
	...
	if (std::
fabs(v.x) > 20000.f || std::
fabs(v.y) > 20000.f || std::
fabs(v.z) > 5000.f)
		return false;
	return true;
}
...
void* PlayerResourcePtr() {
	if (g_ppPlayerResource && Mem::
IsReadable(g_ppPlayerResource, sizeof(void*))) {
		void* p = nullptr;
		__try { p = *g_ppPlayerResource; }
		__except (EXCEPTION_EXECUTE_HANDLER) { TW_SEH_CATCH("bomb.playerResourcePtr"); p = nullptr; }
		if (p && Mem::
Valid(p, 0x660))
			return p;
	}
...
void* FindPlayerResourceEntity() {
	if (!I::
GameEntity || !I::
GameEntity->Instance || !Mem::
Valid(I::GameEntity->Instance, 0x2100))
		return nullptr;
	int nMax = 0;
	__try {
		nMax = I::
GameEntity->Instance->GetHighestEntityIndex();
	} __except (EXCEPTION_EXECUTE_HANDLER) {
		TW_SEH_CATCH("bomb.findResource.maxIndex");
		return nullptr;
	}
	...
	for (int i = 1; i <= lim; ++i) {
		...
		__try {
			entRaw = I::
GameEntity->Instance->Get(i);
		} __except (EXCEPTION_EXECUTE_HANDLER) {
			TW_SEH_CATCH("bomb.findResource.getEntity");
			continue;
		}
		...
		// Fast identity designer check before virtual dump_class_info
		...
		if (Mem::ReadField(Entity, Offset::m_pEntity(), id) && id && Mem::Valid(id, 0x28)) {
			...
			if (Mem::ReadField(id, Offset::m_designerName(), designer) && designer && Mem::IsReadable(designer, 12)) {
				if (!std::strstr(designer, "resource"))
					continue;
			}
		}
...
bool ReadSiteFromResource(void* res, std::
uint32_t off, Vector_t& out) {
	if (!res || !Mem::
Valid(res, off + sizeof(Vector_t)))
		return false;
	Vector_t tmp{};
	__try {
		tmp = *reinterpret_cast<Vector_t*>(reinterpret_cast<std::
uint8_t*>(res) + off);
	} __except (EXCEPTION_EXECUTE_HANDLER) {
		TW_SEH_CATCH("bomb.readSite");
		return false;
	}
...
bool SehGetCenter(uintptr_t fn, void* resource, Vector_t& out) {
	...
	__try {
		...
		TW_SEH_CATCH("bomb.sehGetCenter");
		return false;
	}
}
...
bool Init() {
	...
	if (uintptr_t site = ScanClient("pPlantedC4s",
		...
		g_pPlantedCount = reinterpret_cast<int*>(M::
getAbsoluteAddress(site + 6, 2, 0));
		g_pPlantedSlot = reinterpret_cast<void**>(M::
getAbsoluteAddress(site + 17, 3, 0));
	}
...
getAbsoluteAddress(fn + 0xE, 3, 0));
		...
		if (cand && Mem::
IsReadable(cand, 8)) {
			__try { obj = *cand; } __except (EXCEPTION_EXECUTE_HANDLER) { TW_SEH_CATCH("bomb.readCand"); obj = nullptr; }
		}
...
void RefreshSites() {
	...
	if (gotA || gotB)
		Con::
Ok("Bomb sites A=%d B=%d", g_haveA ? 1 : 0, g_haveB ? 1 : 0);
}
...
int PlantedC4Count() {
	if (!g_pPlantedCount || !Mem::
IsReadable(g_pPlantedCount, sizeof(int)))
		return 0;
	__try {
		const int n = *g_pPlantedCount;
		return (n >= 0 && n < 16) ? n : 0;
	} __except (EXCEPTION_EXECUTE_HANDLER) {
		TW_SEH_CATCH("bomb.plantedCount");
		return 0;
	}
}
...
void* PlantedC4Entity() {
	...
	if (n <= 0 || !g_pPlantedSlot || !Mem::
IsReadable(g_pPlantedSlot, sizeof(void*)))
		return nullptr;
	__try {
...
ValidEntity(e0))
		...
		TW_SEH_CATCH("bomb.plantedC4Entity");
		return nullptr;
	}
}
...
bool TryStartDefuse(void* plantedC4, void* localPawn) {
	if (!Config::
auto_defuse || !g_fnStartDefuse || !plantedC4 || !localPawn)
		return false;
	// Free'd planted / dead pawn mid-round - never call game defuse
	if (!Mem::
ValidEntity(plantedC4) || !Mem::
ValidEntity(localPawn))
		return false;
	__try {
		...
		TW_SEH_CATCH("bomb.tryStartDefuse");
		return false;
	}
}
