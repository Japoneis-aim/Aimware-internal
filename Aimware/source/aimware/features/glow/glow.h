...
// CS2 engine glow ESP (independent of mesh tint):
...
// Glow drives CGlowProperty / scene float4, never mesh vertex colour.
namespace Glow {

[[nodiscard]] inline bool AnyEnabled() noexcept {
	return Config::glow || Config::glow_world_weapons || Config::glow_world_grenades;
}
...
IsReadable(this, kMinSize);
	...
	void* owner() const {
		if (!ok())
			return nullptr;
		return Mem::
ReadFieldOr<void*>(this, 0x18, nullptr);
	}
	...
	void* scene_object_alt() const {
		if (!ok())
			return nullptr;
		return Mem::
ReadFieldOr<void*>(this, 0x20, nullptr);
	}
	...
	void* scene_object() const {
		if (!ok())
			return nullptr;
		return Mem::
ReadFieldOr<void*>(this, 0x28, nullptr);
	}
	int& glow_type() {
		static int s_fb = 0;
		if (!ok()) return s_fb;
		return *reinterpret_cast<int*>(reinterpret_cast<std::
uint8_t*>(this) + 0x30);
	}
	int& glow_range() {
		static int s_fb = 0;
...
uint8_t*>(this) + 0x3C);
	...
	ColorRgba& color_override() {
		...
		if (!ok()) return s_fb;
		return *reinterpret_cast<ColorRgba*>(reinterpret_cast<std::
uint8_t*>(this) + 0x40);
	}
	...
	bool& flashing() {
		...
		if (!ok()) return s_fb;
		return *reinterpret_cast<bool*>(reinterpret_cast<std::
uint8_t*>(this) + 0x44);
	}
 // m_fGlowColor @ +0x8 - float RGB used by scene glow (independent of mesh tint)
	float* glow_color_f3() {
		if (!ok()) return nullptr;
		return reinterpret_cast<float*>(reinterpret_cast<std::
uint8_t*>(this) + 0x8);
	}
	bool& glowing() {
		static bool s_fb = false;
...
// Write Config glow colour -> override + m_fGlowColor.
...
// IDA B499F0: force float4 so mesh pre-fill cannot bleed into glow.
...
std::
int64_t OnApplyGlowScene(CGlowProperty* glow, void* sceneNode,
	std::
int64_t(__fastcall* original)(CGlowProperty*, void*));
