...
isfinite(p.z))
	...
	if (p.x == 0.f && p.y == 0.f && p.z == 0.f)
		return false;
	if (std::
fabs(p.x) > 16384.f || std::
fabs(p.y) > 16384.f || std::
fabs(p.z) > 16384.f)
		return false;
	return true;
}
...
GameEntity->Instance)
		return false;
	auto* pawn = I::
GameEntity->Instance->Get<C_CSPlayerPawn>(h);
	if (!pawn || !Mem::
ValidEntity(pawn))
		return false;
	*out = pawn;
...
ValidEntity(lp))
	...
	const bool doTeamCheck = GameMode::
...
sqrt(vel.x * vel.x + vel.y * vel.y);
		...
		const float t = std::
clamp((speed2d - kMinSpeed) / (kRunSpeed - kMinSpeed + 1.f), 0.f, 1.f);
		...
		const float moved = std::
sqrt(mdx * mdx + mdy * mdy);
...
WorldToScreen(w, s)) {
	...
	if (n >= 6) {
		if (outline)
			dl->AddPolyline(pts, n, outline, thick + 1.8f, ImDrawFlags_Closed);
		dl->AddPolyline(pts, n, col, thick, ImDrawFlags_Closed);
		return;
	}
...
void Install() {
	...
	Con::
Ok("SoundEsp: velocity ground rings (serverside footsteps)");
}
...
sound_esp)
	...
	__try { SampleWalkers(); }
	__except (EXCEPTION_EXECUTE_HANDLER) { TW_SEH_CATCH("soundEsp.sampleWalkers"); }
...
clamp(Config::
sound_esp_ring_size, 0.5f, 3.f);
		const float radius = (kRingStartR + (kRingEndR - kRingStartR) * u) * scale;

		const int a = static_cast<int>(std::
clamp(base.w * fade, 0.f, 1.f) * 255.f);
		if (a < 12)
			continue;

		const ImU32 col = IM_COL32(
			static_cast<int>(std::
clamp(base.x, 0.f, 1.f) * 255.f),
			static_cast<int>(std::
clamp(base.y, 0.f, 1.f) * 255.f),
			static_cast<int>(std::
clamp(base.z, 0.f, 1.f) * 255.f),
			a);
