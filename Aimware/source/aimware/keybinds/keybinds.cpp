...
Keybind::
Keybind(bool& featureFlag, int k, KeyMode m)
	: feature(featureFlag)
	, key(k)
...
Keybinds() {
	...
	keybinds.emplace_back(Keybind(Config::
aimbot, VK_XBUTTON1, KeyMode::
Hold));
	keybinds.emplace_back(Keybind(Config::
autofire, VK_XBUTTON2, KeyMode::
Hold));
	keybinds.emplace_back(Keybind(Config::
triggerbot, VK_MENU, KeyMode::
Hold));
	...
	keybinds.emplace_back(Keybind(Config::mindamage_override, 0, KeyMode::Hold));
	keybinds.emplace_back(Keybind(Config::
thirdperson, VK_MBUTTON, KeyMode::
Toggle));
	keybinds.emplace_back(Keybind(Config::
grenade_helper_capture, VK_F6, KeyMode::
Hold));
	// Always when unbound - Hold+key0 never fires
	keybinds.emplace_back(Keybind(Config::
jumpbug, 0, KeyMode::
Always));
	keybinds.emplace_back(Keybind(Config::
...
bool Keybinds::
isActive(bool& feature) const {
	const Keybind* kb = find(feature);
	return kb ? kb->isActive() : feature;
}

int Keybinds::
getKey(bool& feature) const {
	const Keybind* kb = find(feature);
	return kb ? kb->key : 0;
}

int Keybinds::
getMode(bool& feature) const {
	const Keybind* kb = find(feature);
	return kb ? kb->mode : static_cast<int>(KeyMode::
Always);
}

void Keybinds::
setKey(bool& feature, int key) {
...
void Keybinds::
setMode(bool& feature, int mode) {
	if (Keybind* kb = find(feature)) {
		if (mode < static_cast<int>(KeyMode::
Always) || mode > static_cast<int>(KeyMode::
Toggle))
			mode = static_cast<int>(KeyMode::
Hold);
		kb->mode = mode;
...
aimbot, VK_XBUTTON1);
	setMode(Config::
aimbot, static_cast<int>(KeyMode::
Hold));
	setKey(Config::
autofire, VK_XBUTTON2);
	setMode(Config::
autofire, static_cast<int>(KeyMode::
Hold));
	setKey(Config::
triggerbot, VK_MENU);
	setMode(Config::
triggerbot, static_cast<int>(KeyMode::
Hold));
	setKey(Config::autowall, 0);
	setMode(Config::autowall, static_cast<int>(KeyMode::Always));
	setKey(Config::mindamage_override, 0);
	setMode(Config::mindamage_override, static_cast<int>(KeyMode::Hold));
	setKey(Config::
thirdperson, VK_MBUTTON);
	setMode(Config::
thirdperson, static_cast<int>(KeyMode::
Toggle));
	setKey(Config::
grenade_helper_capture, VK_F6);
	setMode(Config::
grenade_helper_capture, static_cast<int>(KeyMode::
Hold));
	setKey(Config::
jumpbug, 0);
	setMode(Config::
jumpbug, static_cast<int>(KeyMode::
Always));
	setKey(Config::
edgejump, 0);
	setMode(Config::
edgejump, static_cast<int>(KeyMode::
Always));
}
...
void Keybinds::
keyName(int key, char* out, size_t outSize) {
	formatKeyName(key, out, outSize);
}

void Keybinds::
formatKeyName(int key, char* out, size_t outSize) {
	if (!out || outSize == 0)
		return;
	if (key == 0) {
	...
	UINT scan = MapVirtualKeyA((UINT)
key, MAPVK_VK_TO_VSC);
	if (scan) {
		LONG lParam = (LONG)(scan << 16);
		if (GetKeyNameTextA(lParam, out, (int)
outSize) > 0)
			return;
	}
...
pollInputs(bool menuOpen) {
	...
	for (Keybind& k : keybinds) {
		...
		if (static_cast<KeyMode>(k.mode) != KeyMode::
Toggle)
			continue;
...
void Keybinds::
menuMode(bool& feature, float width) {
...
PushID(kb);
	...
	ImGui::
SetNextItemWidth(FitItemWidth(width));
	if (ImGui::
Combo("##mode", &kb->mode, modes, IM_ARRAYSIZE(modes))) {
		if (static_cast<KeyMode>(kb->mode) != KeyMode::
Toggle)
			kb->toggled = false;
	}
	if (ImGui::
IsItemHovered())
		ImGui::
SetTooltip("Always on, hold key, or toggle with key.");
	ImGui::
PopID();
}

void Keybinds::
menuButton(bool& feature, float width) {
	Keybind* kb = find(feature);
	if (!kb) {
		ImGui::
TextDisabled("N/A");
	...
	ImGui::
PushID(reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(kb) ^ 0xB1u));

	// Critical: btnW must never be 0 (auto-size overflows SameLine / tight cards)
	const float btnW = FitItemWidth(width);

	const bool needKey = static_cast<KeyMode>(kb->mode) != KeyMode::
Always;
	if (!needKey) {
		ImGui::
BeginDisabled();
		ImGui::
Button("Always", ImVec2(btnW, 0));
		ImGui::
EndDisabled();
		ImGui::
PopID();
...
IsItemHovered())
			ImGui::
SetTooltip("Click to set key. Right-click to clear.");
...
IsItemClicked(ImGuiMouseButton_Right)) {
		...
		ImGui::
PushStyleColor(ImGuiCol_Button, ImVec4(0.58f, 0.38f, 0.98f, 0.55f));
		ImGui::
PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(0.58f, 0.38f, 0.98f, 0.70f));
		ImGui::
Button("...", ImVec2(btnW, 0));
		ImGui::
PopStyleColor(2);
...
IsItemHovered())
			ImGui::
SetTooltip("Press a key. Esc cancels.");
...
int Keybinds::
listSnapshots(KeybindSnapshot* out, int maxOut) const {
	if (!out || maxOut <= 0)
		return 0;
	int n = 0;
	for (const auto& kb : keybinds) {
		...
		if (&kb.feature == &Config::
grenade_helper_capture)
			s.enabled = kb.feature && Config::
grenade_helper;
		else if (&kb.feature == &Config::
autowall)
			s.enabled = kb.feature
				&& (Config::
autofire_autowall || Config::
trigger_autowall);
		else
			s.enabled = kb.feature;
...
triggerbot)
			...
			const bool af = Config::autofire_autowall;
			const bool tr = Config::trigger_autowall;
			if (tr && !af)
				s.name = "Triggerbot Aw";
			else if (af && !tr)
				s.name = "Autofire Aw";
			else
				s.name = "Autowall";
		}
		else if (&kb.feature == &Config::mindamage_override) {
			s.name = "Min Dmg";
			s.value = Config::mindamage_override_value;
		}
		else if (&kb.feature == &Config::thirdperson)
			s.name = "Third Person";
		else if (&kb.feature == &Config::
grenade_helper_capture)
			s.name = "Grenade Capture";
		else if (&kb.feature == &Config::
edgejump)
			s.name = "Edgejump";
		else
