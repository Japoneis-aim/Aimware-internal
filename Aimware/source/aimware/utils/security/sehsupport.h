...
namespace SehSupport {
 ...
 // Undo RegisterExceptionTable. MUST run before the image is unmapped on
 // unload - stale RUNTIME_FUNCTION entries into freed pages crash the host
 // on the next stack unwind over our range.
 ...
 // Run this module's CRT static initializers (.CRT$XCU) manually.
 // Manual mappers that call DllMain / ManualMapEntry directly skip the CRT
 // entry point, so global objects (Aimware, Config, ...) are never
 // constructed -> vtable-NULL crash on first use. No-op when the CRT already
 // ran (guards on the security-cookie sentinel - never double-inits).
 // Call AFTER RegisterExceptionTable (uses __try).
