...
// Present / CreateMove / glow all call Get after init. Miss-cache emplace
// without a lock is MSVC unordered_map heap corruption.
static SRWLOCK g_schemaLock = SRWLOCK_INIT;
static std::atomic<bool> g_schemaReady{ false };
...
bool Schema::init(const char* ModuleName, int module_type)
{
    ...
    {
        AcquireSRWLockExclusive(&g_schemaLock);
        g_schemaMap.clear();
        g_schemaMap.reserve(dumped_data.size() + 64);
        for (const auto& e : dumped_data)
            g_schemaMap.emplace(e.hashedName, e.uOffset);
        ReleaseSRWLockExclusive(&g_schemaLock);
    }
    g_schemaReady.store(true, std::memory_order_release);
