...
public:
	...
	{
		// alloca() dangles the moment the ctor returns - heap like upstream
		if (nAllocationCount > 0)
			pMemory = static_cast<T*>(malloc(nAllocationCount * sizeof(T)));
	}
	...
	void Grow(const int nCount = 1)
	{
		...
		const int nAllocationRequested = nAllocationCount + nCount;

		// Upstream growth policy, minus the broken nNewAllocationCount loop
		// ((n+req)/2 converges to req-1 -> infinite loop) and the alloca path.
		if (nGrowSize)
	...
	void ConvertToGrowableMemory(int nInitialGrowSize)
	{
		...
		if (nAllocationCount > 0)
		{
			...
			T* pGrowableMemory = static_cast<T*>(malloc(nByteCount));
...
template< class T, class N >
void CUtlMemory<T, N>::
SetExternalBuffer(T* pNewMemory, size_t numElements)
{
	// Blow away any existing allocated memory
	Purge();

	pMemory = pNewMemory;
	nAllocationCount = static_cast<int>(numElements);
...
template< class T, class N >
void CUtlMemory<T, N>::
AssumeMemory(T* pNewMemory, size_t numElements)
{
	// Blow away any existing allocated memory
	Purge();
...
template<class T, class N>
inline CUtlMemory<T, N>::
CUtlMemory(const T* pMemory, size_t numElements) : nAllocationCount(static_cast<N>(numElements))
{
...
void CUtlMemory<T, I>::
Init(size_t nGrowSize /*= 0*/, size_t nInitSize /*= 0*/)
{
	Purge();
	nGrowSize = nGrowSize;
	...
	{
		pMemory = (T*)
malloc(nAllocationCount * sizeof(T));
	}
}

template< class T, class N >
void CUtlMemory<T, N>::
Swap(CUtlMemory<T, N>& mem)
{
...
public:
	...
	[[nodiscard]] T* Base()
	{
		...
		return reinterpret_cast<T*>((reinterpret_cast<std::
uintptr_t>(&pMemory[0]) + nAlignment - 1) & ~(nAlignment - 1));
	}
