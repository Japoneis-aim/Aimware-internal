...
bool ViewMatrix::
WorldToScreen(const Vector_t& position, Vector_t& out) const {
	// Prefer IDA ScreenTransform / live matrix - always correct active view slot.
if (W2S::
WorldToScreen(position, out))
		return true;

	const viewmatrix_t* m = viewMatrix ? viewMatrix : W2S::
Matrix();
	if (!m)
		return false;
...
bool ViewMatrix::
WorldToClip(const Vector_t& position, float& numX, float& numY, float& w) const {
	const viewmatrix_t* m = viewMatrix ? viewMatrix : W2S::
Matrix();
	if (!m)
		return false;
