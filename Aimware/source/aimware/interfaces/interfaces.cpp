...
// Path: tick -> array -> sequence @ +0x5910 -> GetBySeq
CUserCmd* SehGetUserCmdByTickArray(uintptr_t ctrl)
{
...
CUserCmd* SehGetUserCmd(uintptr_t controller)
{
	...
	__try {
		...
		// Path 2: GetCUserCmdTick + GetCUserCmdArray + sequence
		if (CUserCmd* cmd = SehGetUserCmdByTickArray(ctrl))
			return cmd;
...
void Input::init()
{
	...
	// dump: GetCUserCmdArray - IDA 0x180901320 (allocates 150xCUserCmd ring)
	...
	if (!SetupCmd) Con::PatternMiss("client", "Input::SetupCmd");
	if (!GetCUserCmdBySequenceNumber) Con::PatternMiss("client", "Input::GetCUserCmdBySequenceNumber");
	if (!GetCUserCmdArray) Con::PatternMiss("client", "Input::GetCUserCmdArray");
	if (!GetEntityCmdSlot) Con::PatternMiss("client", "Input::GetCUserCmdTick");
	if (!ppUserCmdArrayTable) Con::PatternMiss("client", "Input::UserCmdArrayTable");
	if (!ForceButtonsDown) Con::PatternMiss("client", "Input::ForceButtonsDown");
	if (!pCSGOInput) Con::PatternMiss("client", "Input::pCSGOInput");
	if (!SetViewAngle) Con::PatternMiss("client", "Input::SetViewAngle");
	if (!GetViewAngles) Con::PatternMiss("client", "Input::GetViewAngles");
}
...
bool I::Interfaces::init()
{
 ...
 // mercey material_create - sibling ending `48 8B F2` (KV chams path). dump ADE0 is fallback.
    {
        ...
        if (!CreateMaterial) Con::PatternMiss("materialsystem2", "CreateMaterial");
    }
