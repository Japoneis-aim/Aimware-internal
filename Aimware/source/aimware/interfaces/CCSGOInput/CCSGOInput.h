...
inline constexpr std::size_t kUserCmdStride = 152;
class CTinyMoveStepData{public:
float flWhen;
	MEM_PAD(0x4);
	std::
uint64_t nButton;
...
;
class CExtendedMoveData : public CMoveStepButtons{public:
float flForwardMove;
	float flSideMove;
	float flUpMove;
	std::
int32_t nMouseDeltaX;
	std::
int32_t nMouseDeltaY;
	std::
int32_t nAdditionalStepMovesCount;
...
// IDA: 
// int* GetCUserCmdTick(entity, int* outTick)
...
inline void (*ForceButtonsDown)(void* moveServices, std::
uint64_t buttons) = nullptr;
	// dump: pCSGOInput / CInputPtrGlobal -> CCSGOInput*	
inline void** ppCSGOInput = nullptr;
	inline void* pCSGOInput = nullptr;
	// Global table for GetCUserCmdArray ( GetFirstCUserCmdArray)
