...
// IDA CSource2Client::FrameStageNotify (client 0x180B10FF0) switch cases 3..12.
// VProf: stage 5 = "FrameNetUpdate", stage 6 = "FramePostDataUpdate".
enum EClientFrameStage : int
{
	...
	FRAME_NET_UPDATE_END = 5,       // IDA case 5 - FrameNetUpdate
	FRAME_RENDER_START = 6,         // IDA case 6 - FramePostDataUpdate
	FRAME_RENDER_END = 7,           // IDA case 7
	FRAME_NET_CREATION = 8,
	FRAME_SIMULATE_END = 9,
	FRAME_RESTORE_SERVER_STATE = 10,
};
...
enum ESubtickMoveStepBits : std::uint32_t
{
	...
	MOVESTEP_BITS_PITCH_DELTA = 0x20U,
	MOVESTEP_BITS_YAW_DELTA = 0x40U
};
...
public:
	...
	float flPitchDelta;              // 0x30 (yaw-delta strafer / quantized moves)
	float flYawDelta;                // 0x34
};
...
public:
	...
	// ArenaStringPtr: empty = rdata sentinel; set = (std::string* | tag 2/3)
	void* strMoveCrc;                                       // 0x30
	...
	CSubtickMoveStep* add_subtick_move()
	{
		...
		if (field.pRep) {
			...
			// Torn pRep on leave can report nAllocatedSize = 0x7FFFFFFF.
			// Indexing tElements[cur] then is an arena OOB write -> __fastfail.
			if (cur >= 0 && cap > 0 && cap <= 128 && cur < cap) {
...
public:
	...
	void SetAttack2StartHistoryIndex(std::int32_t idx)
	{
		nAttack2StartHistoryIndex = idx;
		CheckAndSetBits(BITS_ATTACK2START);
	}
};
...
public:
	...
	CCSGOInputHistoryEntryPB* GetInputHistoryEntry(int nIndex)
	{
		...
		if (nIndex < 0 || nIndex >= csgoUserCmd.inputHistoryField.nCurrentSize)
			return nullptr;
		const int cap = csgoUserCmd.inputHistoryField.pRep->nAllocatedSize;
		if (cap <= 0 || cap > 128 || nIndex >= cap)
			return nullptr;
	...
	// narrow=true: only stamp attack entry (seed mode - avoids model flick from
	// wide hist rewrite when solved angle differs from camera).
	void SetAttackHistoryFire(int histIndex, const QAngle_t& angView, const Vector_t& shootPos, bool narrow = false)
	{
		if (!csgoUserCmd.inputHistoryField.pRep)
			return;
		const int count = csgoUserCmd.inputHistoryField.nCurrentSize;
		...
		for (int i = lo2; i <= hi2; ++i) {
			...
			// nRenderTickCount left alone - server side untrust when render == player
			// tick (impossible under real interp) -> VAC insecure.
			if (keepTick > 0) {
	...
	// Seed nospread: stamp EVERY input_history slot with the same
	// unpunched view (roll kept), shoot pos, and player tick. Bind attack1 to
	// the last slot. Zero sv/cl interp so FillGunFireData cannot lerp a
	// neighbor back onto camera angles.
	void StampNospreadHistory(const QAngle_t& angView, const Vector_t& shootPos,
		int playerTick, float playerFrac)
	{
		if (!csgoUserCmd.inputHistoryField.pRep)
			return;
		const int count = csgoUserCmd.inputHistoryField.nCurrentSize;
		if (count <= 0)
			return;
		csgoUserCmd.SetAttack1StartHistoryIndex(count - 1);

		float frac = playerFrac;
		...
		for (int i = 0; i < count; ++i) {
			...
			if (playerTick > 0) {
				e->nPlayerTickCount = playerTick;
				e->flPlayerTickFraction = frac;
				e->SetBits(
					EInputHistoryBits::INPUT_HISTORY_BITS_PLAYERTICKCOUNT
					| EInputHistoryBits::INPUT_HISTORY_BITS_PLAYERTICKFRACTION);
			}
			if (e->sv_interp0) {
				...
				e->sv_interp0->flFraction = 0.f;
				e->SetBits(EInputHistoryBits::INPUT_HISTORY_BITS_SV_INTERP0);
			}
			if (e->sv_interp1) {
				e->sv_interp1->nSrcTick = -1;
				e->sv_interp1->nDstTick = -1;
				e->sv_interp1->flFraction = 0.f;
				e->SetBits(EInputHistoryBits::INPUT_HISTORY_BITS_SV_INTERP1);
			}
			if (e->cl_interp) {
				e->cl_interp->flFraction = 0.f;
				e->SetBits(EInputHistoryBits::INPUT_HISTORY_BITS_CL_INTERP);
			}
		}
	}
};
