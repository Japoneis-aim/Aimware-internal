...
// NetClientInfo - Source2EngineToClient (Pa1nt4r: LocalData.ShootPosition).
...
public:
    char pad_0000[0x4];
 // 0x0000
    int32_t m_nRenderTick;
 // 0x0004
    float m_flRenderTickFraction;
 // 0x0008
    int32_t m_nPlayerTickCount;
 // 0x000C
...
ref(nIndex), 0);
    ...
    static bool info_looks_filled(const c_networked_client_info& out)
    {
        if (out.m_nRenderTick > 0 && out.m_nRenderTick < 0x7FFFFFFF)
            return true;
        const auto* base = reinterpret_cast<const std::
uint8_t*>(&out);
        for (int off = 0x14; off <= 0x28; off += 8) {
            void* p = nullptr;
            std::
memcpy(&p, base + off, sizeof(p));
            if (p && reinterpret_cast<std::
uintptr_t>(p) > 0x10000ull)
                return true;
        }
        return false;
    }
...
memset(&out, 0, sizeof(out));
 // Common indices across builds (compile-time CallVFunc)
        ...
        __try {
            M::
CallVFunc<void, 178U>(this, &out);
            if (info_looks_filled(out))
                return true;
        }
        __except (EXCEPTION_EXECUTE_HANDLER) {
            TW_SEH_CATCH("engineClient.netInfo.178");
        }
        std::
memset(&out, 0, sizeof(out));
        __try {
            M::
CallVFunc<void, 179U>(this, &out, 1);
            if (info_looks_filled(out))
                return true;
        }
        __except (EXCEPTION_EXECUTE_HANDLER) {
            TW_SEH_CATCH("engineClient.netInfo.179");
        }
        std::
memset(&out, 0, sizeof(out));
        __try {
            M::
CallVFunc<void, 177U>(this, &out);
            if (info_looks_filled(out))
                return true;
        }
        __except (EXCEPTION_EXECUTE_HANDLER) {
            TW_SEH_CATCH("engineClient.netInfo.177");
        }
        return false;
    }
    ...
    static bool valid_shoot_vec(const Vector_t& sp)
    {
        if (!std::
isfinite(sp.x) || !std::
isfinite(sp.y) || !std::isfinite(sp.z))
            return false;
        if (sp.x == 0.f && sp.y == 0.f && sp.z == 0.f)
            return false;
        if (std::
fabs(sp.x) > 16384.f || std::
fabs(sp.y) > 16384.f || std::fabs(sp.z) > 16384.f)
            return false;
        return true;
    }
...
uint8_t*>(&info);
        for (int poff = 0x14; poff <= 0x28; poff += 8) {
            void* ld = nullptr;
            std::
memcpy(&ld, base + poff, sizeof(ld));
            if (!ld || !Mem::IsUserPtr(ld))
                continue;
            for (int voff = 0; voff <= 0x18; voff += 4) {
                Vector_t sp{};
                if (!Mem::ReadField(ld, static_cast<std::size_t>(voff), sp))
                    continue;
    ...
    {
        return M::
CallVFunc<void, 60U>(this, width, height);
    }
 ...
 // CEngineClient slots 64/65 (sdk dump) - full path vs short name (de_mirage)
    ...
    {
        return M::
vfunc<const char*, 64U>(this);
    }

    const char* get_level_name_short()
    {
        return M::
vfunc<const char*, 65U>(this);
    }
